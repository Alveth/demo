...
def create_circle(lat, lon, radius_km=1.0, points=50):
    circle = []
    for i in range(points):
        angle = i * (360 / points)
        angle_rad = math.radians(angle)
        dlat = (radius_km / 6371) * math.cos(angle_rad)
        dlon = (radius_km / 6371) * math.sin(angle_rad) / math.cos(math.radians(lat))
        circle.append([lon + math.degrees(dlon), lat + math.degrees(dlat)])
